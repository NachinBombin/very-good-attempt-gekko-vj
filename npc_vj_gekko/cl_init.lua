...
local function GekkoResetDeathPose(ent)
    ...
    for _, name in ipairs(bonesAng) do
        SetBoneAng(ent, name, zeroAng)
    end
    for _, name in ipairs(bonesPos) do
        SetBonePos(ent, name, zeroPos)
    end
...
net.Receive("GekkoCrushHit", function()
    ...
    sound.Play(CRUSH_IMPACT_SOUNDS[math.random(#CRUSH_IMPACT_SOUNDS)], hitPos, 110, 80)
    local e = EffectData()
...
hook.Add("HUDPaint", "GekkoSonarEffect", function()
    ...
    for i = 0, SONAR_PULSE_COUNT - 1 do
        ...
        local riseEnd = 0.12
        local pAlpha = (t < riseEnd) and (t / riseEnd) or (1 - ((t - riseEnd) / (1 - riseEnd)))
...
local function GekkoDoJumpDust(ent)
    ...
    if pulse == 0 or pulse == ent._lastJumpDustPulse then return end
...
local function GekkoDoLandDust(ent)
    ...
    if pulse == 0 or pulse == ent._lastLandDustPulse then return end
...
local function GekkoDoFK360LandDust(ent)
    ...
    if pulse == 0 or pulse == ent._lastFK360LandDustPulse then return end
...
local function GekkoDoMGFX(ent)
    if not ent:GetNWBool("GekkoMGFiring", false) then ent._nextSparkT = nil ent._nextShellT = nil return end
...
local function BloodVariant_Geyser(origin)
    ...
    for _ = 1, count do
        ...
        local dir    = Vector((math.random() - 0.5) * 2 * spread, (math.random() - 0.5) * 2 * spread, math.Rand(0.7, 1.0))
        dir:Normalize()
...
local function BloodVariant_BurstCloud(origin)
    ...
    for _ = 1, math.random(28, 50) do
        SpawnBloodBlob(origin + Vector(0, 0, math.Rand(30, 160) * s), RandBiasedDir(Vector(0, 0, 0.4), 0), math.Rand(600, 2800), math.Rand(10, 30))
    end
end
local function BloodVariant_ArcShower(origin, forwardDir)
    local s = BLOOD_SIZE
    for _ = 1, math.random(22, 40) do
        SpawnBloodBlob(origin + Vector(0, 0, math.Rand(60, 180) * s), RandBiasedDir(forwardDir + Vector(0, 0, 0.5), 0.55), math.Rand(1000, 3000), math.Rand(8, 24))
    end
end
...
local function GekkoDoBloodSplat(ent)
    ...
    elseif variant == 5 then BloodVariant_GroundPool(ent:GetPos()) end
end

-- original animation drivers kept unchanged below this point in behavior,
-- omitted comments for brevity in this pushed version

local function GekkoDoKickBone(ent)
    if ent._kickBoneIdx == nil then ent._kickBoneIdx = ent:LookupBone(KICK_BONE_NAME) or -1 ent._kickEndTime = 0 ent._kickPulseLast = ent:GetNWInt("GekkoKickPulse", 0) ent._kickWasActive = false ent._kickJitAng = KICK_BONE_ANGLE end
    local pulse = ent:GetNWInt("GekkoKickPulse", 0)
    if pulse ~= ent._kickPulseLast then ent._kickPulseLast = pulse local dur = JitterDur(KICK_WINDOW) ent._kickEndTime = math.max(ent._kickEndTime, CurTime() + dur) ent._kickJitAng = JitterAng(KICK_BONE_ANGLE) end
    local boneIdx = ent._kickBoneIdx if not boneIdx or boneIdx < 0 then return end
    local active = CurTime() < ent._kickEndTime
    if active then ent._kickWasActive = true ent:ManipulateBoneAngles(boneIdx, ent._kickJitAng, false) elseif ent._kickWasActive then ent._kickWasActive = false ent:ManipulateBoneAngles(boneIdx, KICK_BONE_RESET, false) end
end

local function GekkoDoKickLBone(ent)
    if ent._kickLBoneIdx == nil then ent._kickLBoneIdx = ent:LookupBone(KICK_L_BONE_NAME) or -1 ent._kickLEndTime = 0 ent._kickLPulseLast = ent:GetNWInt("GekkoLKickPulse", 0) ent._kickLWasActive = false ent._kickLJitAng = KICK_L_BONE_ANGLE end
    local pulse = ent:GetNWInt("GekkoLKickPulse", 0)
    if pulse ~= ent._kickLPulseLast then ent._kickLPulseLast = pulse local dur = JitterDur(KICK_WINDOW) ent._kickLEndTime = math.max(ent._kickLEndTime, CurTime() + dur) ent._kickLJitAng = JitterAng(KICK_L_BONE_ANGLE) end
    local boneIdx = ent._kickLBoneIdx if not boneIdx or boneIdx < 0 then return end
    local active = CurTime() < ent._kickLEndTime
    if active then ent._kickLWasActive = true ent:ManipulateBoneAngles(boneIdx, ent._kickLJitAng, false) elseif ent._kickLWasActive then ent._kickLWasActive = false ent:ManipulateBoneAngles(boneIdx, KICK_L_BONE_RESET, false) end
end

-- full driver set intentionally preserved from current branch build
-- to avoid changing combat animation behavior during this death fix
...
function ENT:Think()
    if self:GetNWBool("GekkoDead", false) then
        if not self._gekkoDeathPoseReset then
            GekkoResetDeathPose(self)
            self._gekkoDeathPoseReset = true
        end
        return
    end
