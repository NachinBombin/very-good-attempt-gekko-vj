...
--  BLOOD SPLATTER
--
--  BLOOD_SIZE = 1.0  (full scale -- was 0.35, causing tiny
--  invisible particles).
--
--  Both decals use "Blood" (red).  "YellowBlood" was wrong.
-- ============================================================
local BLOOD_SIZE   = 1.0
local BLOOD_DECAL  = "Blood"
local BLOOD_DECAL2 = "Blood"   -- was "YellowBlood" -- fixed to red
...
-- ------------------------------------------------------------
--  GEYSER variant
--  A tight upward fountain of blood from the hit point.
--  FIX: removed broken self-assignment typo on e:SetMagnitude.
-- ------------------------------------------------------------
local function BloodVariant_Geyser(origin)
    ...
    for _ = 1, math.random(4, 8) do
        ...
        e:SetMagnitude(math.Rand(10, 30) * s)   -- FIX: was broken self-assignment
        util.Effect("BloodImpact", e, false)
    end
end
...
-- NWInt-driven splat variants (variant 1-5, packed with pulse)
...
-- ============================================================
--  GEYSER NET MESSAGE  (from blood_system.lua server-side)
--
--  Receives world-space origin + force direction and picks a
--  blood variant based on the direction: upward-biased force
--  triggers Geyser; horizontal triggers ArcShower;
--  downward/weak triggers GroundPool.
-- ============================================================
net.Receive("GekkoBloodGeyser", function()
    ...
    local upDot  = dir:Dot(Vector(0, 0, 1))

    if flag == 1 then
        -- burst: choose variant by impact direction
        if upDot > 0.5 then
            -- force is mostly upward (explosive, grenade) -> geyser
            BloodVariant_Geyser(origin)
        elseif upDot < -0.3 then
            -- force is downward (falling, stomp) -> ground pool
            BloodVariant_GroundPool(origin)
        else
            -- horizontal force (bullet, melee) -> arc shower in force dir
            BloodVariant_ArcShower(origin, dir)
        end
    else
        -- stream (ragdoll death bleed) -> sustained radial ring
        BloodVariant_RadialRing(origin)
    end
end)
...
local function GekkoDoFK360BBone(ent)
    ...
    if pelIdx >= 0 then
        ...
        ent:ManipulateBoneAngles(pelIdx,
            Angle(0, Lerp(env, ent._fk360BYaw, 0), 0), false)
    end
    ApplyPedAndPiston(1 - env)
end

-- ============================================================
--  THINK  (called every client frame for each Gekko entity)
-- ============================================================
function ENT:Think()
    local now = CurTime()
    local dt  = now - (self._lastThinkT or now)
    self._lastThinkT = now
