...
local function FireFK360(self, closestTarget, fwd, dot)
    ...
    print(string.format("[GekkoCrush] FK360 wind-up  target=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), dot, next))

    local selfRef, tgtRef = self, closestTarget
    timer.Simple(0.25, function()
        if not IsValid(selfRef) or not IsValid(tgtRef) then return end
        local fwdRef = selfRef:GetForward()
        local impulse = (fwdRef + Vector(0, 0, 0.4)):GetNormalized() * A.impulse
        CrushDamageEnt(selfRef, tgtRef, A.dmg, impulse)

        print(string.format("[GekkoCrush] FK360 HIT1  target=%s  dot=%.2f  pulse=%d",
            tgtRef:GetClass(), dot, next))
    end)
...
local function FireFK360B(self, closestTarget, fwd, dot)
    ...
    print(string.format("[GekkoCrush] FK360B wind-up  target=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), dot, next))
...
local function FireHeadbutt(self, closestTarget, fwd)
    ...
    local selfRef, tgtRef, fwdRef = self, closestTarget, fwd
    timer.Simple(0.18, function()
        if not IsValid(selfRef) or not IsValid(tgtRef) then return end
        CrushDamageEnt(selfRef, tgtRef, A.dmg, (fwdRef + Vector(0,0,0.3)):GetNormalized() * A.impulse)
    end)
end
...
local function FireKick(self, kickTarget, fwd)
    ...
    local selfRef, tgtRef, fwdRef = self, kickTarget, fwd
    timer.Simple(0.22, function()
        if not IsValid(selfRef) or not IsValid(tgtRef) then return end
        local toT  = (tgtRef:GetPos() - selfRef:GetPos()):GetNormalized()
        local dotT = math.Clamp(fwdRef:Dot(toT), 0.5, 1.0)
        CrushDamageEnt(selfRef, tgtRef, A.dmg * dotT,
...
local function FireSpinKick(self, closestTarget, fwd, dot, inCone)
    ...
    print(string.format("[GekkoCrush] SPINKICK wind-up  target=%s  inCone=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), tostring(inCone), dot, next))

    local selfRef, tgtRef = self, closestTarget
    timer.Simple(0.35, function()
        if not IsValid(selfRef) or not IsValid(tgtRef) then return end
        local dir = (tgtRef:GetPos() - selfRef:GetPos()):GetNormalized()
        CrushDamageEnt(selfRef, tgtRef, A.dmg, (dir + Vector(0,0,0.4)):GetNormalized() * A.impulse)

        print(string.format("[GekkoCrush] SPINKICK HIT  target=%s  inCone=%s  dot=%.2f  pulse=%d",
            tgtRef:GetClass(), tostring(inCone), dot, next))
    end)
end
    ...
    timer.Simple(A.hit_t, function()
        ...
        if IsValid(tr.Entity) and (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) then
            CrushDamageEnt(selfRef, tr.Entity, A.dmg,
                (fwdRef + Vector(0,0,0.25)):GetNormalized() * A.impulse)
        end
    end)
end
...
local function FireHeelHook(self)
    ...
    timer.Simple(A.hit_t2, function()
        ...
        local sweepDir = (fwdRef - rightRef * math.tan(hookRad)):GetNormalized()
        ...
        if IsValid(tr.Entity) and (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) then
            CrushDamageEnt(selfRef, tr.Entity, A.dmg2,
                (sweepDir + Vector(0,0,0.15)):GetNormalized() * A.impulse2)
        end
    end)
end
...
local function FireAxeKick(self, closestTarget, dot)
    ...
    print(string.format("[GekkoCrush] AXEKICK  target=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), dot, next))
    ...
    timer.Simple(A.hit_t, function()
        ...
        if IsValid(tr.Entity) and (tr.Entity:IsNPC() or tr.Entity:IsPlayer()) then
            -- Axe kick drives downward: impulse carries a -z component
...
local function FireRAxeKick(self, closestTarget, dot)
    ...
    print(string.format("[GekkoCrush] RAXEKICK  target=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), dot, next))
...
local function FireJumpKick(self, closestTarget, fwd, dot)
    ...
    print(string.format("[GekkoCrush] JUMPKICK  target=%s  dot=%.2f  pulse=%d",
        closestTarget:GetClass(), dot, next))
