...
--   * FIX: Removed duplicate local Dodge_EnterCrouch / Dodge_ExitCrouch.
--     BeginSlide now calls ENT:Dodge_EnterCrouch (crouch_system.lua) so
--     all crouch state is managed from one authoritative place.
--   * FIX: Flinching killed BEFORE and AFTER SetSchedule(SCHED_NONE) in
--     BeginSlide. VJ Base processes the schedule on the same engine tick
--     and can set Flinching before GekkoUpdateAnimation next runs, which
--     gated out GeckoCrouch_Update and caused the post-dodge up-down flicker.
-- ============================================================
...
-- The old local Dodge_EnterCrouch and Dodge_ExitCrouch duplicates
-- have been removed: they bypassed crouch_system.lua's EnterCrouch
-- and were never the authoritative path for state management.
-- ============================================================
...
local function BeginSlide(ent, slideDir, withCrouch)
    ...
    if withCrouch then
        -- FIX: route through ENT:Dodge_EnterCrouch (crouch_system.lua) instead of
        -- the old local duplicate. This ensures crouch_system's EnterCrouch is called,
        -- _gekkoCrouchHoldUntil is set with correct priority logic, and all state is
        -- managed from one authoritative place.
