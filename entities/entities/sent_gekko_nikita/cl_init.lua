...
--  Visuals: identical to sent_npc_trackmissile
--    PRE-IGNITION : yellow sparks + white steam
--    ENGINE ON    : grey exhaust plume + fire beam + heatwave
-- ============================================================
include( "shared.lua" )
...
function ENT:Draw()
    ...
    -- PRE-IGNITION : sparks + white steam
    if not engineOn then
        for i = 1, 8 do
            ...
            if sp then
                ...
                sp:SetStartAlpha( 255 )  sp:SetEndAlpha( 0 )
                sp:SetStartSize( math.Rand( 3, 7 ) )  sp:SetEndSize( 0 )
                sp:SetRoll( math.Rand( 0, 360 ) )  sp:SetRollDelta( math.Rand( -8, 8 ) )
                sp:SetColor( math.random(230,255), math.random(200,245), 80 )
                sp:SetLighting( false )
                sp:SetGravity( Vector( 0, 0, -120 ) )  sp:SetAirResistance( 20 )
            end
        end
        for i = 1, 3 do
            ...
            if sm then
                ...
                sm:SetStartAlpha( math.Rand( 160, 220 ) )  sm:SetEndAlpha( 0 )
                sm:SetStartSize( math.Rand( 20, 36 ) )  sm:SetEndSize( math.Rand( 90, 160 ) )
                sm:SetRoll( math.Rand( 0, 360 ) )  sm:SetRollDelta( math.Rand( -1.5, 1.5 ) )
                sm:SetColor( 255, 255, 255 )  sm:SetLighting( false )
                sm:SetAirResistance( 45 )  sm:SetGravity( Vector( 0, 0, 70 ) )
            end
        end
        return
    end

    -- ENGINE ON : dynamic light
    ...
    if dlight then
        ...
        dlight.b          = 0
        dlight.Brightness = 1  dlight.Decay = 0.1  dlight.Size = 2048
    ...
    for i = 1, 3 do
        ...
        if particle then
            ...
            particle:SetStartAlpha( math.Rand( 35, 65 ) )  particle:SetEndAlpha( 0 )
            particle:SetStartSize( math.Rand( 12, 14 ) )  particle:SetEndSize( math.Rand( 25, 35 ) )
            particle:SetRoll( math.Rand( 0, 360 ) )  particle:SetRollDelta( math.Rand( -1, 1 ) )
            particle:SetColor(
                math.Rand( 185, 205 ), math.Rand( 185, 205 ), math.Rand( 180, 205 )
            )
